...
class Animal:
    ...
    def sight_entities(self) -> (["Food"], ["Rabbit"], ["Fox"], ["Water"]):
        ...
        for food in self.world.food:
            if self != food and self._in_sight(food):
        ...
        for rabbit in self.world.rabbits:
            if self != rabbit and self._in_sight(rabbit):
                rabbitlist.append(rabbit)

        # Get foxes around self
        foxlist = []
        for fox in self.world.foxes:
            if self != fox and self._in_sight(fox):
                foxlist.append(fox)
    ...
    def roam_move(self) -> None:
        ...
        # Check if valid move
        while (
            self.world.in_bounds((new_x, new_y)) != True
            and contains(self.world.watercells, lambda x: x.x == new_x and x.y == new_y)
            != True
