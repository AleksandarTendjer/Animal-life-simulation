...
class World:
    ...
    def __init__(
        ...
        for _ in range(20):
            ...
            self.rabbits.append(
                Rabbit(self, (rand_pos.x, rand_pos.y), self._random_speed())
            )  # 2.5

        for _ in range(12):
            rand_pos = random.choice(self.landcells)
            self.foxes.append(
                Fox(self, (rand_pos.x, rand_pos.y), self._random_speed())
            )  # 3 self._random_pos()
    ...
    def _random_pos(self) -> (float, float):
        ...
        return (random.uniform(0, self.size[0]), random.uniform(0, self.size[1]))
    ...
    def __repr__(self) -> str:
        return "size={}, rabbits={}, foxes={}, food={}".format(
            self.size, len(self.rabbits), len(self.foxes), len(self.food)
        )

    def _classify_terrain(self):
        for cell in self.cells:
            if (
                cell.biome == NoiseMapBiome.FOREST
                or cell.biome == NoiseMapBiome.GRASSLAND
            ):
