...
class Rabbit(Animal):
    ...
    def move(self) -> None:
        ...
        if foxlist:
            ...
            if not self.world.in_bounds((new_x, new_y)):
                ...
                t = atan2(
                    self.world.size[0] / 2 - self.pos[1],
                    self.world.size[1] / 2 - self.pos[0],
                )
                new_x = self.pos[0] + (self.speed * cos(t))
                new_y = self.pos[1] + (self.speed * sin(t))
            ...
            if (self.target is not None) and (self.target in self.world.food):
                ...
                if dist_to_target <= self.speed:
                    ...
                    if (
                        self.eat_count % 2 == 0
                        and self.eat_count != self._food_checkpoint
                    ):
            ...
            if (self.target is not None) and (self.target in self.world.rabbits):
                ...
                if dist_to_target <= self.speed:
                    ...
                    # else:
                    self.world.rabbits.append(
                        Rabbit(
                            self.world,
                            self.pos,
                            variance(self.speed, self.target.speed, 1.0),
                        )
                    )
                ...
                if dist_to_target <= self.speed:
                    ...
                    if (
                        self.drink_count % 2 == 0
                        and self.drink_count != self._water_checkpoint
                    ):
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        screen.blit(
            rabbit_im, (self.pos[0] - RABBIT_SIZE / 2, self.pos[1] - RABBIT_SIZE / 2)
        )
